...
export default {
  ...
  methods: {
    ...
    generateUrl() {
      ...
      const minStarsQuery = minStars ? `+stars:>=${minStars}` : ''
      const fromDateQuery = fromDate ? `+created:>=${fromDate}` : ''
      const toDateQuery = toDate ? `+created:<=${toDate}` : ''

      return `https://api.github.com/search/repositories?q=language:${this.language}${minStarsQuery}${fromDateQuery}${toDateQuery}&sort=stars&per_page=6`
    }
  }
}
</script>
